...
class SharedWorkflowContractTests(unittest.TestCase):
    ...
    def test_uses_discovery_and_mirror_matrices(self) -> None:
        ...
        self.assertIn("workflow-deadline-epoch: ${{ steps.workflow_deadline.outputs.workflow-deadline-epoch }}", text)
        self.assertIn("discover-units=", text)
        self.assertIn('my $discovery_units = scalar( @{ $config->{projects} || [] } );', text)
        self.assertIn('my $source_group_paths = $namespace->{source_group_paths};', text)
        self.assertIn('matrix: ${{ fromJSON(needs.bootstrap.outputs.discover-matrix) }}', text)
        self.assertIn("min_discovery_jobs = 10", text)
        self.assertIn("max_matrix_jobs = 250", text)
        self.assertIn("job_count = min(max(total_units, min_discovery_jobs), max_matrix_jobs)", text)
    ...
    def test_uses_config_specific_target_pat_secret(self) -> None:
        ...
        self.assertNotIn("GL_GROUPS_SOURCE_USERNAME", text)
        self.assertNotIn("GL_GROUPS_SOURCE_TOKEN", text)
    ...
    def test_workflow_runtime_is_capped_below_six_hours(self) -> None:
        ...
        self.assertIn('deadline_epoch="$(( $(date +%s) + 350 * 60 ))"', text)
        self.assertIn("workflow-deadline-epoch: ${{ needs.bootstrap.outputs.workflow-deadline-epoch }}", text)
        self.assertIn("WORKFLOW_DEADLINE_EPOCH: ${{ needs.bootstrap.outputs.workflow-deadline-epoch }}", text)
        self.assertIn("WORKFLOW_DEADLINE_EPOCH: ${{ needs.plan.outputs.workflow-deadline-epoch }}", text)
        self.assertIn('timeout --signal=TERM "${remaining_seconds}s" perl shared/.github/scripts/glab_groups.pl \\', text)
        self.assertIn('timeout --signal=TERM "${remaining_seconds}s" python3 shared/.github/scripts/post_run_analytics.py \\', text)
        self.assertIn('timeout --signal=TERM "${remaining_seconds}s" python3 - <<\'PY\'', text)
        self.assertIn("workflow deadline exceeded before mirror started", text)
    ...
    def test_step_summary_is_bounded(self) -> None:
        text = WORKFLOW.read_text(encoding="utf-8")
        self.assertIn("Publish plan summary", text)
        self.assertIn('append_line(parts, "## Plan Overview")', text)
        self.assertIn("source_filter = plan.get(\"source_group_filter\") or {}", text)
        self.assertIn('append_line(parts, f"- configured_source_groups: {source_filter.get(\'configured_source_groups\', 0)}")', text)
        self.assertIn('append_line(parts, f"- target_project_namespaces: {source_filter.get(\'target_project_namespaces\', plan.get(\'total_groups\', 0))}")', text)
        self.assertIn('append_line(parts, f"- planned_actions: sync={counts.get(\'sync\', 0)} skip={counts.get(\'skip\', 0)} fail={counts.get(\'fail\', 0)}")', text)
        self.assertIn('append_line(parts, "### Largest Source Groups By Planned Targets")', text)
        self.assertIn("Publish batch summary", text)
        self.assertIn('append_line(parts, f"## Batch Shard {shard_index} Overview")', text)
        self.assertIn('append_line(parts, f"- skipped: total={counts.get(\'skipped\', 0)} archived={archived_skipped} refs_matched={refs_matched_skipped} other={len(other_skipped)}")', text)
        self.assertIn('append_repo_details(parts, "Other Skipped", other_skipped, "reason")', text)
        self.assertIn('append_repo_details(parts, "Failed", failed, "error")', text)
        ...
        self.assertIn('append_line(parts, f"- missing_source_groups: {source_filter.get(\'missing_source_groups\', len(missing_source_groups))}")', text)
        self.assertIn('append_line(parts, f"- excluded_source_groups: {source_filter.get(\'excluded_source_groups\', len(excluded_source_groups))}")', text)
        self.assertIn('append_line(parts, "### Excluded Source Groups")', text)
        self.assertIn('append_line(parts, f"- skipped: total={result_counts.get(\'skipped\', 0)} archived={archived_skipped} refs_matched={refs_matched_skipped} other={len(other_skipped)}")', text)
        self.assertIn('append_items(parts, "Other Skipped", other_skipped, "reason")', text)
        self.assertIn('append_items(parts, "Result File Errors", input_failures, "error")', text)
        self.assertIn("see workflow artifacts for full JSON outputs", text)
        self.assertNotIn("total_target_groups", text)
